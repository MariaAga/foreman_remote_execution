...
export const ExecutionOrderingField = ({ isRandomizedOrdering, setValue }) => (
  <FormGroup
    ...
    labelIcon={helpLabel(
      __(
        'Execution ordering determines whether the jobs should be executed on hosts in alphabetical order or in randomized order.Ordered - executes the jobs on hosts in alphabetical order Randomized - randomizes the order in which jobs are executed on hosts'
      ),
      'effective-user-password'
    )}
    isInline
